...
const AppointmentModal = ({ property, onClose }) => {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setIsSubmitting(true); if (!appointmentDate || !appointmentTime) { toast.error('Огноо, цагаа сонгоно уу.'); setIsSubmitting(false); return; } const appointmentDateTime = `${appointmentDate}T${appointmentTime}:00`; try { const response = await fetch('/api/appointments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ propertyId: property._id, dateTime: appointmentDateTime }), }); const data = await response.json(); if (data.success) { toast.success('Таны цагийг амжилттай товлолоо!'); onClose(); } else { toast.error(data.message || 'Цаг товлоход алдаа гарлаа.'); } } catch (error) { toast.error('Гэнэтийн алдаа гарлаа. Дахин оролдоно уу.'); } finally { setIsSubmitting(false); } };
  const inputStyle = "block w-full rounded-lg border-gray-300 shadow-sm focus:border-zolGold focus:ring-zolGold py-3 px-4 transition-all text-zolDark bg-gray-50";
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"><div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full font-poppins animate-in fade-in"><button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-zolDark transition-colors" aria-label="Close Modal"><X size={24} /></button><h2 className="font-playfair text-2xl font-bold text-zolGreen mb-6">Танилцах цаг товлох</h2><form onSubmit={handleSubmit} className="space-y-5"><div><label className="flex items-center gap-2 text-sm font-medium text-zolDark/80 mb-2"><Calendar size={16} /> Огноо</label><input type="date" value={appointmentDate} onChange={(e) => setAppointmentDate(e.target.value)} min={new Date().toISOString().split("T")[0]} className={inputStyle} required /></div><div><label className="flex items-center gap-2 text-sm font-medium text-zolDark/80 mb-2"><Clock size={16} /> Цаг</label><input type="time" value={appointmentTime} onChange={(e) => setAppointmentTime(e.target.value)} className={inputStyle} required /></div><button type="submit" disabled={isSubmitting} className="w-full bg-zolGold hover:bg-zolGold/90 text-white font-semibold py-3 rounded-lg shadow-md transition-transform hover:scale-105 disabled:opacity-50">{isSubmitting ? 'Илгээж байна...' : 'Цаг товлох'}</button></form></div></div>
  );
};
