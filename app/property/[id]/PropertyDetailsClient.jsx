...
import { BedDouble, Bath, LandPlot, MapPin, X, Calendar, Clock, Phone, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="relative bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full font-poppins animate-in fade-in">
        ...
        <h2 className="font-playfair text-2xl font-bold text-zolGreen mb-6">
          Танилцах цаг товлох
        </h2>
...
export default function PropertyDetailsClient({ property, relatedProperties }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        router.push('/all-properties'); // Redirect to all properties instead of homepage
        router.refresh();
      } else {
  ...
  const formatPrice = (price) =>
    typeof price !== 'number' ? "Үнэ тохиролцоно" : `${price.toLocaleString()}₮`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1 space-y-8">
              <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 hover:shadow-2xl transition-shadow">
                ...
                <p className="font-playfair text-4xl font-bold text-zolGold mt-6">
                ...
                <div className="grid grid-cols-3 gap-6 text-center border-t border-b mt-6 py-6">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-sm font-medium">{property.area.toLocaleString()} м²</span>
                  </div>
                  {/* Optional: Add Bed & Bath if available */}
                  {property.bedrooms && (
                    <div className="flex flex-col items-center gap-1">
                      <BedDouble className="text-zolGreen" />
                      <span className="text-sm font-medium">{property.bedrooms} өрөө</span>
                    </div>
                  )}
                  {property.bathrooms && (
                    <div className="flex flex-col items-center gap-1">
                      <Bath className="text-zolGreen" />
                      <span className="text-sm font-medium">{property.bathrooms} угаалгын</span>
                    </div>
                  )}
                </div>
                ...
                {property.number && (
                  <div className="flex items-center bg-green-50 p-4 rounded-lg my-6 border border-green-100 shadow-sm">
                    ...
                    <span className="text-green-800 font-semibold">
                      Холбогдох дугаар: {property.number}
                    </span>
                  </div>
                )}
